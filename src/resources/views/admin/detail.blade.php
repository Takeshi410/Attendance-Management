...
<form action="{{ route('admin.correction', ['id' => $attendance->id]) }}" method="post">
    ...
    <table class="table table--detail {{ (($attendance->latestAttendanceAdjustment?->is_approval) === false) && (($attendance->latestAttendanceAdjustment?->is_admin) === false) ? 'readonly' : '' }}">
        ...
        <tr class="table__row">
            ...
            <td colspan="3" class="table__cell"><textarea class="table--detail .table__cell--remarks" name="remarks" rows="3" >{{ old('remarks', $attendance->latestAttendanceAdjustment->remarks ?? '') }}</textarea></td>
        </tr>
    </table>
    ...
    <div class="content__submit">
        ...
        @if (
            (($attendance->latestAttendanceAdjustment?->is_admin) === false) &&
            (($attendance->latestAttendanceAdjustment?->is_approval) === false)
        )
            <p class="content__submit--message">*承認待ちのため修正はできません。</p>
        @else
            <button class="content__submit--btn">修正</button>
        @endif
    </div>
</form>
